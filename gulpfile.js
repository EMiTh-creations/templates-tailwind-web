...
function styles() {
    return gulp
        ...
        .pipe(plugins.sass())
        .pipe(plugins.postcss([autoprefixer(), cssnano(), cssmqpacker()]))
...
function watch() {
    ...
    gulp.watch([paths.styles.tailwind.src, "tailwind.config.js"], styles_tailwind);
    gulp.watch(paths.images.src, images);
    gulp.watch(paths.static.src, static);
}

//define build process, whether tasks run in parallel or series.
var build = gulp.series(clean, static, gulp.parallel(styles, styles_tailwind, scripts, images));
